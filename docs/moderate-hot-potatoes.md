...
11. Every non-Close action control uses `btn-17 hp-button`; its animated effect is clipped to its own button. Close remains the separately defined `btn-74` control.
...
3. `#InstructionsDiv`, `#MainDiv`, and `#FeedbackDiv`, plus a finished `.btn-74` Close control.
...
5. Short-answer textareas need accessible names. If no recognizable Close control exists, the normalizer adds one at the wrapper footer. ([Dictation profile](/home/eagles/dockerz/efast-copy/scripts/modernize-hot-potatoes-pages.cjs:61))
